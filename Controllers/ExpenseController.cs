...
namespace GroupExpenseManagement01.Controllers
{
    ...
    public class ExpenseController : Controller
    {
        ...
        {
            decimal[] totals = CommonFuntions.UserWiseCurrenciesTotal();
            ViewData["INRAmount"] = totals[0];
            ViewData["USDAmount"] = totals[1];
            ViewData["EURAmount"] = totals[2];
        ...
        {
            if(c["GroupID"].ToString() == "" || c["GroupID"].ToString() == null)
            {
                TempData["ErrorMessage"] = "Select Group!!";
        ...
        public decimal GetReceivableAmout(int GroupID)
        {
            ...
            using (SqlConnection sqlConnection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                {
                    ...
                    sqlCommand.Parameters.Add("@CurrencyID", SqlDbType.Int).Value = CV.CurrencyID();
